...
class Page
{
    ...
    protected function showLeftEllipsis(){
        ...
        if(count($list)==$this->getPagingNo() && $list[1] > 2){
            if($list[1]==3){
                return $this->replace('{leftEllipsis}',$this->linkCode(2));
            }
    ...
    protected function showRightEllipsis(){
        ...
        if(count($list) == $this->getPagingNo() && $list[$this->getPagingNo()]< $this->getTotalPageNo()-1){
            if($list[$this->pagingNo]==($this->getTotalPageNo()-2)){
                return $this->replace('{rightEllipsis}',$this->linkCode($this->getTotalPageNo()-1));
            }
    ...
    protected function getPagingCode(){
        ...
        foreach($this->getPagingList() as $val){
            $html.= $this->linkCode($val);
        }
        return $html;
    }
    protected function getNextPageCode(){
        return $this->linkCode($this->getNextPageNo(), $this->nextText);
    }
    ...
    protected function getPrePageCode(){
        return $this->linkCode($this->getPrePageNo(), $this->preText);
    }
    ...
    protected function getIndexPageCode(){
        return $this->linkCode(1);
    }
    protected function getLastPageCode(){
        return $this->linkCode($this->getTotalPageNo());
    }
    ...
    protected function linkCode($num, $text=''){
        $html = '<a ';
